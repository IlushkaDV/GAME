extends Area2D
# Интерактивный объект, который служит основой для установленных турелей.
# На него можно щелкнуть, чтобы установить турель (когда слот пустой) или отобразить интерфейс,
# показывающий действия, которые можно выполнить с ней.
# Когда турели уничтожены, базовый слот остается на месте.


var turret: Turret  # турель, назначенная этому слоту
