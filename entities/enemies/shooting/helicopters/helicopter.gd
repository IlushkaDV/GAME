...
@onready var shadow := $Shadow as AnimatedSprite2D  # Готовая ссылка на тень как анимированный спрайт
@onready var rotor := $Rotor as AnimatedSprite2D  # Готовая ссылка на ротор как анимированный спрайт
@onready var explosion := $Explosion as AnimatedSprite2D  # Готовая ссылка на взрыв как анимированный спрайт


func _ready() -> void:
	super()
	# убедиться, что все анимированные спрайты синхронизированы
	_sync_animations()


func _physics_process(delta: float) -> void:
	super(delta)
	# обработать вращение всех спрайтов
